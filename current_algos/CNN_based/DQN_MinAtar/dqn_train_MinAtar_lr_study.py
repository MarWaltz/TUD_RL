...
def train(env_str, double, dqn_weights=None, seed=0, device="cpu"):
    ...
    for total_steps in range(TIMESTEPS):
        ...
        if (total_steps + 1) % EPOCH_LENGTH == 0 and (total_steps + 1) > agent.upd_start_step:
            ...
            np.save(file=f"{agent.logger.output_dir}/epi_ret_list.npy", arr=np.array([epi_ret_list]))
            np.save(file=f"{agent.logger.output_dir}/epi_ret_step_list.npy", arr=np.array([epi_ret_step_list]))
