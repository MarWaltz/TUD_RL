...
def train(c: ConfigFile, agent_name: str):
    ...
    for wrapper in c.Env.wrappers:
        ...
        test_env: gym.Env = get_wrapper(
            wrapper, env=test_env, **wrapper_kwargs)
    ...
    agent.print_params(agent.n_params, mode=1)
    ...
    for total_steps in range(c.timesteps):
        ...
        if total_steps < c.act_start_step:
            a = np.random.uniform(
                low=agent.action_low, high=agent.action_high, size=agent.num_actions)
        else:
            if "LSTM" in agent.name:
                a = agent.select_action(
                    s=s, s_hist=s_hist, a_hist=a_hist, hist_len=hist_len)
        ...
        if (total_steps + 1) % c.epoch_length == 0 and (total_steps + 1) > c.upd_start_step:
            ...
            if "LSTM" in agent.name:
                ...
                agent.logger.log_tabular(
                    "Actor_ExtMemory", with_min_and_max=False)
                agent.logger.log_tabular(
                    "Critic_CurFE", with_min_and_max=False)
                agent.logger.log_tabular(
                    "Critic_ExtMemory", with_min_and_max=False)
