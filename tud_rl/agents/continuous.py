...
class DDPGAgent(BaseAgent):
    def __init__(self, c: ConfigFile, agent_name, init_critic=True):
        ...
        if self.state_type == "image":
            raise Exception(
                "Currently, image input is not supported for continuous action spaces.")
        ...
        # action normalizer
        self.act_normalizer = Action_Normalizer(
            action_high=self.action_high, action_low=self.action_low)
        ...
        self.n_params = self._count_params(
            self.actor), self._count_params(self.critic)
    ...
    def polyak_update(self):
        ...
        for target_p, main_p in zip(self.target_actor.parameters(), self.actor.parameters()):
            target_p.data.copy_(self.tau * main_p.data +
...
class LSTMDDPGAgent(BaseAgent):
    def __init__(self, c: ConfigFile, agent_name, init_critic=True):
        ...
        self.use_past_actions = getattr(c.Agent, agent_name)[
            "use_past_actions"]
    ...
    def select_action(self, s, s_hist, a_hist, hist_len):
        ...
        s = torch.tensor(s, dtype=torch.float32).view(
            1, self.state_shape).to(self.device)
        s_hist = torch.tensor(
...
class LSTMSACAgent(BaseAgent):
    def __init__(self, c: ConfigFile, agent_name):
        ...
        if self.temp_tuning:
            ...
            self.log_temperature = torch.zeros(
                1, requires_grad=True, device=self.device)

            # define temperature optimizer
            self.temp_optimizer = optim.Adam(
                [self.log_temperature], lr=self.lr_temp)
    ...
    def select_action(self, s, s_hist, a_hist, hist_len):
        ...
        # forward pass
        if self.mode == "train":
            a, _, _ = self.actor(s, s_hist, a_hist, hist_len,
    ...
    def _compute_target(self, s2_hist, a2_hist, hist_len2, r, s2, d):
        ...
        with torch.no_grad():
            ...
            y = r + self.gamma * (1 - d) * (Q_next -
                                            self.temperature * target_logp_a)
        return y
    ...
    def train(self):
        ...
        curr_a, curr_a_logprob, act_net_info = self.actor(
            s=s, s_hist=s_hist, a_hist=a_hist, hist_len=hist_len, deterministic=False, with_logprob=True)
...
class SACAgent(BaseAgent):
    def __init__(self, c: ConfigFile, agent_name, init_critic=True):
        ...
        if self.net_struc_actor is not None:
            warnings.warn(
                "The net structure of the Gaussian actor cannot be controlled via the config-spec for SAC.")
    ...
    def _compute_target(self, r, s2, d):
        with torch.no_grad():
            ...
            # target actions come from current policy (no target actor)
            target_a, target_logp_a = self.actor(
                s2, deterministic=False, with_logprob=True)
    ...
    def train(self):
        ...
        # get current actions via actor
        curr_a, curr_a_logprob = self.actor(
            s, deterministic=False, with_logprob=True)
...
class TQCAgent(SACAgent):
    ...
    def _compute_target(self, r, s2, d):
        with torch.no_grad():
            ...
            # target actions come from current policy (no target actor)
            next_new_action, next_log_pi = self.actor(
                s2, deterministic=False, with_logprob=True)
            ...
            y = r + (1-d) * self.gamma * (sorted_z_part -
                                          self.temperature * next_log_pi)
        return y
    ...
    def _quantile_huber_loss(self, quantiles, y):
        ...
        pairwise_delta = y[:, None, None, :] - \
            quantiles[:, :, :, None]  # Reshape to
    ...
    def train(self):
        ...
        actor_loss = (self.temperature * curr_a_logprob -
                      current_mean_z).mean()
